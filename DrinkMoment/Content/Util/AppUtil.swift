...
struct FileHelper<T: Codable> {
    ...
    static func getValueFor(_ key: Key, defaultValue: T) -> T {
        return UserDefaults.standard.getObject(T.self, forKey: key.rawValue) ?? defaultValue
    }
    
    static func setValueFor(_ key:  Key, value: T) {
